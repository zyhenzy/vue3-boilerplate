  ...
  >
    <a-descriptions title="详细信息" bordered>
      ...
      <a-descriptions-item label="核心武将" :span="3">
        <a-tag v-for='(hero,index) in accountDetail.showHeroList' :key='index'>{{hero.advance_num}}{{hero.name}}</a-tag>
      </a-descriptions-item>
      <a-descriptions-item label="粉武" :span="3">
        <a-tag v-for='(weapon,index) in accountDetail.weaponRedList' :key='index'>{{weapon.name}}{{weapon.featureName}}</a-tag>
      </a-descriptions-item>
